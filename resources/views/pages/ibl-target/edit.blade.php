                ...
                <div class="col-md-12 grid-margin stretch-card">
                    <div class="card">
                        <div class="card-body">
                            ...
                            <form class="forms-sample" action="{{ route('ibl-target.update', $iblTarget->id) }}" method="POST">
                                @csrf
                                @method('PUT')
                                <div class="row">
                                    <div class="col-sm-4">
                                            ...
                                            <select class="form-select mb-3" name="village_id" required>
                                                ...
                                                @foreach ($villages as $village)
                                                    <option value="{{ $village->id }}"
                                                        {{ $iblTarget->village_id == $village->id ? 'selected' : '' }}>
                                                        {{ $village->name }}</option>
                                                @endforeach
                                            </select>
                                            @error('village_id')
                                    ...
                                    <div class="col-sm-4">
                                        <div class="mb-3">
                                            ...
                                            <input type="number" value="{{ $iblTarget->sum_boys }}"
                                ...
                                <a href="{{ route('ibl-target.index') }}" class="btn btn-secondary">Batal</a>
                            </form>
                        </div>
                    </div>
                </div>
